...
$(function () {
    ...
    // $('#submit-form-paystack').on('click', function () {
    //     $("#paystack_formId").submit();
    // });
$(document).on('click', '#submit-form-paystack', function (e) {
    e.preventDefault();
    $('#paystack_formId').submit();
});
...
$('#paystack_formId').parsley().on('form:submit', function () {
    onConfirmWhenAddEditForPaystack();
    return false;
});

function onConfirmWhenAddEditForPaystack() {
    $.ajax({
        type: 'POST',
        url: "{{ route('backend.PaystackSettingsUpdate') }}",
        data: $('#paystack_formId').serialize(),
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
