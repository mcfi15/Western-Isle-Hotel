...
class SettingsController extends Controller
{
    ...
    // ================= Paystack =================
    $paystack_data = Tp_option::where('option_name', 'paystack')->get();
    $paystack_id = '';
    foreach ($paystack_data as $row){
        $paystack_id = $row->id;
    }

    $paystack_data_list = [];
    if($paystack_id != ''){
        $paystackData = json_decode($paystack_data);
        $paystackObj = json_decode($paystackData[0]->option_value);
        $paystack_data_list['paystack_public_key'] = $paystackObj->paystack_public_key;
        $paystack_data_list['paystack_secret_key'] = $paystackObj->paystack_secret_key;
        $paystack_data_list['paystack_currency'] = $paystackObj->paystack_currency;
        $paystack_data_list['ismode_paystack'] = $paystackObj->ismode_paystack;
        $paystack_data_list['isenable_paystack'] = $paystackObj->isenable_paystack;
    }else{
        $paystack_data_list = [
            'paystack_public_key' => '',
            'paystack_secret_key' => '',
    ...
    return view('backend.payment-methods', compact(
        'stripe_data_list',
        'paypal_data_list',
        'razorpay_data_list',
        'mollie_data_list',
...
public function PaystackSettingsUpdate(Request $request)
{
    ...
    $isenable_paystack = $request->has('isenable_paystack') ? 1 : 0;
    $ismode_paystack   = $request->has('ismode_paystack') ? 1 : 0;
    ...
    $option_value = [
        ...
        'ismode_paystack'     => $ismode_paystack,
        'isenable_paystack'   => $isenable_paystack,
    ];

    $data = [
        'option_name'  => 'paystack',
        'option_value' => json_encode($option_value),
    ];

    $row = Tp_option::where('option_name', 'paystack')->first();
...
// 	public function PaystackSettingsUpdate(Request $request){
// 		// dd('PAYSTACK CONTROLLER HIT', $request->all());
//     $res = [];

//     $paystack_public_key = $request->input('paystack_public_key');
//     $paystack_secret_key = $request->input('paystack_secret_key');
//     $paystack_currency = $request->input('paystack_currency');
//     $is_enable = $request->input('isenable_paystack');
//     $is_mode = $request->input('ismode_paystack');

	

//     $isenable_paystack = ($is_enable == 'true' || $is_enable == 'on') ? 1 : 0;
//     $ismode_paystack = ($is_mode == 'true' || $is_mode == 'on') ? 1 : 0;
...
//             'msgType' => 'error',
//             'msg' => $errors->first('paystack_public_key')
//         ]);
//     }
...
//         return response()->json([
//             'msgType' => 'error',
//             'msg' => $errors->first('paystack_secret_key')
//         ]);
//     }

//     if($errors->has('paystack_currency')){
//         return response()->json([
//             'msgType' => 'error',
//             'msg' => $errors->first('paystack_currency')
//         ]);
//     }
...
//     $gData = Tp_option::where('option_name', 'paystack')->get();
//     $id = '';
//     foreach ($gData as $row){
//         $id = $row['id'];
//     }
...
//         $res['msg'] = $response ? __('Saved Successfully') : __('Data insert failed');
//     }else{
//         $response = Tp_option::where('id', $id)->update($data);
//         $res['msgType'] = $response ? 'success' : 'error';
//         $res['msg'] = $response ? __('Updated Successfully') : __('Data update failed');
//     }
